...
modelname<- c("pool.full.model", "pool.noho.model",  "pool.nocv.model",
...
for (h in 1: length(depend.var))   {
        ...
        for (j in 1: length(modelname))  {       
                ...
                source("./Code/Parameter_Vectors2.R")
                ...
                if (isTRUE(grepl("pool", modelname[j])))  {
                        ...
                        if (isTRUE(grepl("nocv", modelname[j]))) 
                        {par<-pool.nocv.par
                        par_lo<-pool.nocv.par.lo
                        par_hi<-pool.nocv.par.hi}
                ...
                #                         if (isTRUE(grepl("nocv", modelname[j]))) 
                #                         { par<-site.nocv.par
                #                           par_lo<-site.nocv.par.lo
                #                           par_hi<-site.nocv.par.hi}
                #                         
                ...
                var <- list(mean = "predicted", x=depend.var[h],HO=data$HO, CV=data$CV, 
                ...
                write_results(results,paste("./Results/Model Fits/Abu2/",dep.var,"_","PIUNs","_",modelname[j],".txt", sep=""))
        }
}
