...
if(respuesta == "abundance") {
        parameters <-read.delim ("./Results/BestModel_Abu3.txt", dec=".", header=T)
} else if (respuesta == "richness") {
        parameters <- read.delim ("./Results/BestModel_Ric3.txt", dec=".", header=T)
}
...
seqDW<-seq(from=quantile(ocells$DW, probs = 0.1), to= quantile(ocells$DW, probs = 0.9), by= 1.5)
...
figura <- function (depvar, expvar, maxiabu, pos="none") { 
        ...
        for (i in 1: length (depvar)) {   
                ...
                DWa<- rep(NA, times=length(depvar))        
                DWb<- rep(NA, times=length(depvar))
                ...
                DWa[i] <- parameters[( parameters$Variable==depvar[i]),17]
                DWb[i] <- parameters[( parameters$Variable==depvar[i]),18]
                ...
                if (expname=="seqHO")  {output[,i]<-pool.full.model(PotRichness[i],HOa= HOa[i],HOb= HOb[i], DWa= DWa[i], DWb=DWb[i],MGa[i],MGb[i],SHa[i],SHb[i],
                                                                HO=seqHO, DW=mean(ocell$DW), MG=mean(ocell$MG), SH=mean(ocell$SH))}
                else if (expname=="seqDW") {output[,i]<-pool.full.model(PotRichness[i],HOa= HOa[i],HOb= HOb[i], DWa= DWa[i], DWb=DWb[i],MGa[i],MGb[i],SHa[i],SHb[i],
                                                                HO=mean(ocell$HO),DW=seqDW,  MG=mean(ocell$MG), SH=mean(ocell$SH)) } 
                else if (expname=="seqMG") {output[,i]<-pool.full.model(PotRichness[i],HOa= HOa[i],HOb= HOb[i], DWa= DWa[i], DWb=DWb[i],MGa[i],MGb[i],SHa[i],SHb[i],
                                                              HO=mean(ocell$HO), DW=mean(ocell$DW),  MG=seqMG, SH=mean(ocell$SH))}
                else if (expname=="seqSH") {output[,i]<-pool.full.model(PotRichness[i],HOa= HOa[i],HOb= HOb[i], DWa= DWa[i], DWb=DWb[i],MGa[i],MGb[i],SHa[i],SHb[i],
                                                               HO=mean(ocell$HO), DW=mean(ocell$DW), MG=mean(ocell$MG), SH=seqSH) }               
...
figura_rel <- function (depvar, expvar, pos="none") {    
        ...
        for (i in 1: length (depvar)) {        
                ...
                if (expname=="seqHO")  {output[,i]<-pool.full.model(1,HOa= HOa[i],HOb= HOb[i], DWa= 1000, DWb=100000,MGa=1000,MGb=100000,SHa=1000,SHb=1000000,
                                                                    HO=seqHO, DW=mean(ocells$DW), MG=mean(ocells$MG), SH=mean(ocells$SH))}
                else if (expname=="seqDW") {output[,i]<-pool.full.model(1,HOa= 1,HOb= 100000, DWa= DWa[i], DWb=DWb[i],MGa=1,MGb=1000000,SHa=1,SHb=1000000,
                                                                        HO=mean(ocells$HO),DW=seqDW,  MG=mean(ocells$MG), SH=mean(ocells$SH)) } 
                else if (expname=="seqMG") {output[,i]<-pool.full.model(1,HOa= 1000,HOb= 1000000, DWa= 1000, DWb=1000000,MGa[i],MGb[i],SHa=1000,SHb=1000000,
                                                                        HO=mean(ocells$HO), DW=mean(ocells$DW),  MG=seqMG, SH=mean(ocells$SH))}
                else if (expname=="seqSH") {output[,i]<-pool.full.model(1,HOa= 1,HOb= 100000, DWa= 1, DWb=100000,MGa=1,MGb=1000000,SHa=SHa[i],SHb=SHb[i],
                                                                        HO=mean(ocells$HO), DW=mean(ocells$DW), MG=mean(ocells$MG), SH=seqSH) }               
...
figura_3D <- function (depvar, expvar1,expvar2, zgir, xgir, lay=c(2,2)) {
        ...
        for (i in 1: length (depvar)) {        
                ...
                else if (expname1=="seqHO" & expname2 =="seqDW") 
                        {HO.effect <- exp(-0.5*((output$x-HOa[i])/HOb[i])^2)
                        MG.effect <- 1
                        DW.effect <- exp(-0.5*((output$y-DWa[i])/DWb[i])^2)
                        SH.effect <- 1}
                ...
                else if (expname1=="seqMG" & expname2 =="seqDW") 
                        {HO.effect <- 1
                ...
                else if (expname1=="seqDW" & expname2 =="seqSH") 
                        {HO.effect <- 1
                        MG.effect <- 1
                        DW.effect <- exp(-0.5*((output$x-DWa[i])/DWb[i])^2)
                        SH.effect <- exp(-0.5*((output$y-SHa[i])/SHb[i])^2)}
                ...
                output[,i+2]<- 1 * HO.effect *  DW.effect *  MG.effect * SH.effect
