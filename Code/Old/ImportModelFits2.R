...
write.table(modelcomp, file = "./Results/ModelComparison_Abu2.txt", sep="\t", col.names=NA)
...
par_pool_full <- ldply(fullnames,function(extract)    {
        ...
        CVa <- read.table(extract, skip=14, nrows=1, header=F,  sep="\t")
        CVb <- read.table(extract, skip=15, nrows=1, header=F,  sep="\t")
        ...
        File <- extract
        parameters <- cbind(PotRichness[,2],HOa[,2],HOb[,2],CVa[,2],CVb[,2],
...
par_pool_noho <- ldply(nohonames,function(extract)    {
        PotRichness <- read.table(extract, skip=11, nrows=1, header=F,  sep="\t")
        CVa <- read.table(extract, skip=12, nrows=1, header=F,  sep="\t")
        CVb <- read.table(extract, skip=13, nrows=1, header=F,  sep="\t")
        ...
        File <- extract
        parameters <- cbind(PotRichness[,2],0,10000000,CVa[,2],CVb[,2],
                            MGa[,2],MGb[,2],SHa[,2],SHb[,2],lambda[,2], File[1])
        return(parameters) })

par_pool_nocv <- ldply(nocvnames,function(extract)    {
        PotRichness <- read.table(extract, skip=11, nrows=1, header=F,  sep="\t")
        HOa <- read.table(extract, skip=12, nrows=1, header=F,  sep="\t")
        HOb <- read.table(extract, skip=13, nrows=1, header=F,  sep="\t")
        MGa <- read.table(extract, skip=14, nrows=1, header=F,  sep="\t")
...
write.table(parameters, file = "./Results/ModelParameters_Abu2.txt", sep="\t", col.names=NA)
